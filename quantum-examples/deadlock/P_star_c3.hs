import BellKAT.QuantumPrelude

hubFull :: QBKATTest
hubFull =
        hasSubset ["C" ~ "H", "C" ~ "H", "C" ~ "H"]
    ||* hasSubset ["A" ~ "H", "C" ~ "H", "C" ~ "H"]
    ||* hasSubset ["B" ~ "H", "C" ~ "H", "C" ~ "H"]
    ||* hasSubset ["A" ~ "H", "B" ~ "H", "C" ~ "H"]

hubHasRoom :: QBKATTest
hubHasRoom = notB hubFull

cHasRoom :: QBKATTest
cHasRoom = hasNotSubset ["C" ~ "H", "C" ~ "H", "C" ~ "H"]

cOnlyDeadlock :: QBKATTest
cOnlyDeadlock = hasSubset ["C" ~ "H", "C" ~ "H", "C" ~ "H"]

p :: QBKATPolicy
p =
    while ("A" /~? "C" &&* "B" /~? "C" &&* notB cOnlyDeadlock)
        (   (   ite (hubHasRoom &&* "A" /~? "H") (ucreate ("A", "H")) mempty
            <||>
                ite (hubHasRoom &&* "B" /~? "H") (ucreate ("B", "H")) mempty
            <||>
                ite (hubHasRoom &&* cHasRoom) (ucreate ("C", "H")) mempty
            )
        <>
            (   swap "H" ("A", "C")
            <.>
                swap "H" ("B", "C")
            )
        )

networkCapacity :: NetworkCapacity QBKATTag
networkCapacity =
    [ "A" ~ "H"
    , "B" ~ "H"
    , "C" ~ "H"
    , "C" ~ "H"
    , "C" ~ "H"
    , "A" ~ "C"
    , "B" ~ "C"
    ]

nb :: NetworkBounds QBKATTag
nb = def
    { nbCapacity = Just networkCapacity
    , nbOperationTiming = InstantaneousOps
    }

actionConfig :: ProbabilisticActionConfiguration
actionConfig =
    PAC
        { pacTransmitProbability = []
        , pacCreateProbability = []
        , pacCreateWerner = []
        , pacUCreateProbability =
            [ (("A", "H"), 1/4)
            , (("B", "H"), 1/3)
            , (("C", "H"), 1/3)
            ]
        , pacSwapProbability =
            [ ("H", 1/2) ]
        , pacUCreateWerner =
            [ (("A", "H"), 90/100)
            , (("B", "H"), 95/100)
            , (("C", "H"), 95/100)
            ]
        , pacCoherenceTime =
            [ ("A", 100)
            , ("B", 100)
            , ("C", 100)
            , ("H", 200)
            ]
        , pacDistances =
            [ (("A", "H"), 1)
            , (("B", "H"), 1)
            , (("C", "H"), 1)
            ]
        }

main :: IO ()
main =
    let ev = "A" ~~? "C" ||* "B" ~~? "C"
    in qbkatMainD actionConfig nb ev p mempty
